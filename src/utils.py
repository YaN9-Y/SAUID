...
nyu_cmap = np.load('./checkpoints/cmap.npy')
...
class _SimpleSegmentationModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x, imme_feature = self.classifier(features)
        x = F.interpolate(x, size=input_shape, mode='bilinear', align_corners=False)
        return x, imme_feature
...
def decode_segment_result(input_tensor, dataset='cityscapes'):
    assert dataset.lower() in ['cityscapes','nyu']
    if dataset.lower() == 'cityscapes':
        decode_fn = Cityscapes.decode_target
        input_tensor = input_tensor.max(1)[1].cpu().numpy()
        colorized_preds = torch.from_numpy(decode_fn(input_tensor)/255.0).permute(0,3,1,2)
        return colorized_preds

    elif dataset.lower() == 'nyu':
        input_tensor = input_tensor[0].cpu().numpy().transpose(1, 2, 0)
        input_tensor = torch.from_numpy(nyu_cmap[input_tensor.argmax(axis=2).astype(np.uint8)]).unsqueeze(0).permute(0,3,1,2)/255.0
        return input_tensor
...
def get_dark_channel(input):
    return torch.min(input, dim=1, keepdim=True)[0]

def get_local_patch(x):
    # input x,y: NCHW
    im_H, im_W = x.shape[2:4]
    m_H, m_W = im_H//2, im_W//2

    m_start_H = np.random.randint(0,im_H - m_H)
    m_start_W = np.random.randint(0,im_W-m_W)

    return m_start_H, m_start_H+m_H, m_start_W, m_start_W+m_W


def warp(x, flo, padding_mode='border', requires_mask=False):
    B, C, H, W = x.size()
    ...
    if requires_mask:
        mask = torch.ones(output.size(), device='cuda').float()
        mask = F.grid_sample(mask, vgrid, padding_mode='zeros', align_corners=True)
        mask[mask < 0.999] = 0
        mask[mask > 0] = 1

        return output, mask ## mask : 1 for meaningful region, 0 for padded region
