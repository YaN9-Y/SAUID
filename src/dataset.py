...
class Dataset(torch.utils.data.Dataset):
    def __init__(self, config, crop_size,  hazy_flist, clean_flist=None, segment_flist=False,  clean_path=None, hazy_path=None, augment=True, split='unpair'):
        super(Dataset, self).__init__()
        self.augment = augment
        self.config = config
        assert split in ['unpair','pair_train', 'pair_test', 'hazy', 'clean']
        ...
        #self.segment_data = self.load_flist(segment_flist)
        if str.lower(self.config.DATASET) in ['real']:
            self.jitter = transforms.ColorJitter(brightness=(0.8, 1), contrast=(self.config.LOW_CONTRAST, self.config.HIGH_CONTRAST), saturation=(0.4, 1))

        # print(self.noisy_data)
    ...
    def __len__(self):
        if self.split in ['pair_test', 'hazy']:
    ...
    def load_item(self, index):
        # load image
            ...
            if self.split in ['hazy']:
                ...
                while (True):
                    ...
                    if min(np.array(img_clean).shape[0:2])>=self.config.CROP_SIZE:
                        #print(min(np.array(img_clean).shape[0:2]))
                        break
                ...
                #
                # while (True):
                #     segment_index = int(np.random.random() * len(self.segment_data))
                #     img_segment= Image.open((self.segment_data[segment_index]))
                #     if min(np.array(img_segment).shape[0:2]) > self.config.CROP_SIZE:
                #         break
                    # else:
                    #     print('loop')
                ...
                img_noisy = self.convert_to_rgb(img_noisy)
                img_clean = self.convert_to_rgb(img_clean)
                # img_segment = self.convert_to_rgb(img_segment)
            ...
            elif self.split in ['pair_test']:
                # print(3)

                if str.lower(self.config.SEG_DATASET) == 'cityscapes':
                    img_noisy = Image.open(self.noisy_data[index])
                    #img_clean = Image.open(self.clean_data[index//3])
                    
                    basename = '_'.join(os.path.basename(self.noisy_data[index]).split('_')[:-3])+'.png'
                    
                    folder_path = '/'.join((self.clean_data[0]).split('/')[:-1])
                    #img_clean = Image.open('_'.join(self.noisy_data[index].split('_')[:-3])+'.png')
                    clean_img_path = os.path.join(folder_path,basename)

                    img_clean = Image.open(clean_img_path)

                elif str.lower(self.config.SEG_DATASET) == 'nyu':
                    img_noisy = Image.open(self.noisy_data[index])
                    basename = '_'.join(os.path.basename(self.noisy_data[index]).split('_')[:-1])+'.png'
                    folder_path = '/'.join((self.clean_data[0]).split('/')[:-1])
                    clean_img_path = os.path.join(folder_path,basename)
                    img_clean = Image.open(clean_img_path)

                print(self.noisy_data[index])
                print(clean_img_path)
            ...
            # elif self.split in ['pair_test_segment']:
            #     print(3)
            #
            #     img_hazy = Image.open(self.noisy_data[index])
            #     path_segment = self.get_gt_dehaze_path(self.noisy_data[index])
            #
            #     img_segment = Image.open(path_segment)
            #
            #     img_hazy = self.convert_to_rgb(img_hazy)
            #     img_segment = self.convert_to_rgb(img_segment)
            #
            #     if img_segment.size != img_hazy.size:
            #         img_segment = TF.center_crop(img_segment, img_hazy.size[::-1])
            #
            #     img_segment = TF.to_tensor(img_segment)
            #     img_hazy = TF.to_tensor(img_hazy)
            #
            #
            #
            #     # except ValueError:
            ...
            #
            #
            #     return img_hazy, img_segment #, gt_grad

            # elif self.split in ['pair_train']:
            #
            #     img_noisy = Image.open(self.noisy_data[index])
            #     img_clean = self.get_gt_dehaze_path(self.noisy_data[index])
            #
    ...
    def get_gt_dehaze_path(self, path):
    ...
    def get_gt_segment_path(self,path):
        pass
